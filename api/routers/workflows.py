...
class RankingRequest(BaseModel): query: str
class ExtractRequest(BaseModel): s3_prefix: Optional[str] = None; s3_object_key: Optional[str] = None

class QuestionParams:
    def __init__(self, query: str = Form(...), user_id: str = Form("default_user"), model_name: Optional[str] = Form(None), doc_type: Optional[str] = Form(None), product_type: Optional[str] = Form(None), files: List[UploadFile] = File(default=[])):
        self.query, self.user_id, self.model_name, self.doc_type, self.product_type, self.files = query, user_id, model_name, doc_type, product_type, files
...
def get_orchestrator(req: Request): return req.app.state.orchestrator
def get_rag_pipeline(req: Request): return req.app.state.rag_pipeline
...
async def ask_question(params: QuestionParams = Depends(), pipeline: RAGPipeline = Depends(get_rag_pipeline)):
    ...
    return pipeline.answer_question(query=params.query, user_id=params.user_id, model_name=params.model_name, files=file_data, doc_type=params.doc_type, product_type=params.product_type)
...
def get_agent_types():
    ...
    return load_from_json("/home/muthu/PycharmProjects/procwise_agents/prompts/agent_definitions.json")